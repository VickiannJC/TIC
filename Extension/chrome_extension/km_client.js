(function () {
  ...
  async function ensureHandshake() {
    ...
    // 1) Claves ECC del plug-in (generar/importar)
    await loadOrCreatePluginKeyPair();
    // 2) Registrar public key del plug-in en KM (auth_plugin_key)
    await registerPluginPublicKey();
    // 3) Derivar clave de canal compartida ECDH + HKDF
  ...
  const KMClient = {
     ...
     */
    async init({ kmBaseUrl, userId, pluginId }) {
      ...
      // No forzamos handshake aquí, pero podrías hacerlo:
